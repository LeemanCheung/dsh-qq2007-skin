...
4. Install one stylesheet scoped under `body[data-dsh-qq2007-active="true"]`.
5. Add a factual local visual-state strip with an original embedded SVG buddy and local clock.
6. Register a General-settings row that switches between the retro theme and `system`.
7. Dispose theme registration, CSS, DOM, interval, listeners, and state markers with the owning Cordis fiber.
...
`src/client.js`, `src/skin.css`, and `assets/retro-buddy.svg` are deterministic sources. `npm run build` embeds CSS and SVG into the standard `window.__ModuleLoader__.load(...)` client bundle at `lib/client.js`.
